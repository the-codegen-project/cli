...
/**
 * Helper to check stderr doesn't contain actual errors.
 * Allows Node.js deprecation warnings (like punycode) which are not actual errors.
 */
function expectNoActualErrors(stderr: string): void {
  // Filter out Node.js deprecation warnings
  const actualErrors = stderr
    .split('\n')
    .filter(line => line.trim() !== '')
    .filter(line => !line.includes('DeprecationWarning'))
    .filter(line => !line.includes('node --trace-deprecation'))
    .join('\n');
