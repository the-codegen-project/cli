...
type UpdatePetResponse = APet | string | string | string;

export function unmarshal(json: any): UpdatePetResponse {
...
export function marshal(payload: UpdatePetResponse): string {
  if(payload instanceof APet) {
  return payload.marshal();
}
  return JSON.stringify(payload);
}
export function unmarshalByStatusCode(json: any, statusCode: number): {error?: string, statusCode: number, payload?: UpdatePetResponse} {
  switch(statusCode) {
    case 200:
...
export const theCodeGenSchema = {"type":"object","oneOf":[{"title":"a Pet","description":"A pet for sale in the pet store","type":"object","required":["name","photoUrls"],"properties":{"id":{"type":"integer","format":"int64"},"category":{"title":"Pet category","description":"A category for a pet","type":"object","properties":{"id":{"type":"integer","format":"int64"},"name":{"type":"string","pattern":"^[a-zA-Z0-9]+[a-zA-Z0-9\\.\\-_]*[a-zA-Z0-9]+$"}},"xml":{"name":"Category"}},"name":{"type":"string","example":"doggie"},"photoUrls":{"type":"array","xml":{"name":"photoUrl","wrapped":true},"items":{"type":"string"}},"tags":{"type":"array","xml":{"name":"tag","wrapped":true},"items":{"title":"Pet Tag","description":"A tag for a pet","type":"object","properties":{"id":{"type":"integer","format":"int64"},"name":{"type":"string"}},"xml":{"name":"Tag"}}},"status":{"type":"string","description":"pet status in the store","deprecated":true,"enum":["available","pending","sold"]}},"xml":{"name":"Pet"},"$id":"updatePet_Response_200"},{"type":"string","description":"Invalid ID supplied","$id":"updatePet_Response_400"},{"type":"string","description":"Pet not found","$id":"updatePet_Response_404"},{"type":"string","description":"Validation exception","$id":"updatePet_Response_405"}],"$id":"UpdatePetResponse","$schema":"http://json-schema.org/draft-07/schema"};
