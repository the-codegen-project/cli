...
type FormatUuid = string;

export function unmarshal(json: string): FormatUuid {
  return JSON.parse(json) as FormatUuid;
}
export function marshal(payload: FormatUuid): string {
  return JSON.stringify(payload);
}
export const theCodeGenSchema = {"type":"string","$schema":"http://json-schema.org/draft-07/schema","format":"uuid","description":"String with UUID format","$id":"FormatUuid"};
