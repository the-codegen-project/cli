 ...
 */
type IntegerPlain = number;

export function unmarshal(json: string): IntegerPlain {
  return JSON.parse(json) as IntegerPlain;
}
export function marshal(payload: IntegerPlain): string {
  return JSON.stringify(payload);
}
export const theCodeGenSchema = {"type":"integer","$schema":"http://json-schema.org/draft-07/schema","description":"A plain integer type","$id":"IntegerPlain"};
